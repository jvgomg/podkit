 ...
 *     no in-VM applier.
 *
 * Shared between `scripts/vm-doctor.ts` (read-side: hashes host, compares
 * to VM) and `scripts/harness.ts` (write-side: hashes host, seals into VM
 * after `harness:setup`). Lifting the file glob + hashing primitive into a
 * module keeps the two paths in lockstep — neither can drift from the
 * other without a unit test catching it.
 *
 * @module
 */
...
/**
 * In-VM path where the sealed baseline hash lives. Written by
 * `harness:setup` after successful provisioning and binary install; read
 * by `vm:doctor` to detect drift.
 *
