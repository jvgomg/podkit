# @podkit/device-testing-daemon

Userspace daemon that synthesises iPod-shaped USB devices on Linux
...
├── package.json            # @podkit/device-testing-daemon (workspace member)
├── tsconfig.json
...
`package.json` declares `@podkit/device-testing` as a regular workspace
dependency. `parseSidecar` and the sidecar types are imported by package
name; `bun build --compile` resolves the workspace symlink and bundles
the resulting tree into a single self-contained binary. The compiled
binary is fully relocatable — it has no runtime dependency on the
...
`test-packages/device-testing/src/personas/sidecar.ts` and re-exported
from the `@podkit/device-testing` index.
...
machinery just by being added to `test-packages/device-testing/src/personas/`.
...
1. **A non-standard USB descriptor shape.** Add the new fields to
   `SidecarUsbDescriptor` in `test-packages/device-testing/src/personas/sidecar.ts`,
...
```
bun test test-packages/device-testing-daemon/src/__tests__/
```
