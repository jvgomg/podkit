...
- [ ] **Publish to the Starlight site.** Today the architecture docs
  live in `docs/architecture/`. Eventually they'll mount in the
...
|------------------------|--------------------------------|------------------------------------------------------------|
| **Architecture docs**  | `docs/architecture/`      | Slow-moving settled conventions. Updated when a refactor changes the convention. |
| **Doc-NNN journals**   | `backlog/docs/`                | Living rough-edges + open question logs. Refreshed often.  |
| **ADRs**               | `docs/adr/`                         | Frozen-at-decision-time, status evolves (Accepted/Superseded). |
