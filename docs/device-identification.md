...
The hardware inventory and per-device inquiry results live in [`docs/test-devices.md`](./test-devices.md). That file is the authoritative log — this section summarises the inquiry-method findings that shaped the boundary research.
...
Hardware inventory + per-device inquiry log: [`docs/test-devices.md`](./test-devices.md). XML captures live in `docs/sysinfo-captures/`. Generation coverage gaps noted in the "Generation Coverage Analysis" section of `test-devices.md` (no nano 5G hash72; no Classic 6G; no nano 1G; no nano 6G).
