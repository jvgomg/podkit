...
`known_hosts` to stop matching, and see
[Host keys after a recreate](#host-keys-after-a-recreate) before accepting the
new one.
...
Once the doctor passes on a rebooted box, **seal it**:
...
With no `PODKIT_PVE_*` keys configured these print the `qm` they would have run
and exit non-zero, so a hand-built box stays on the same code path. `status`
still answers, because a substrate that replies over ssh is a substrate that is
running — that is an observation, not a guess.
...
### Host keys after a recreate

Recreating a guest regenerates its SSH host keys, so `known_hosts` stops
matching. **The pool-scoped token cannot tell you the new key.** Reading it
means guest-exec, which is `VM.GuestAgent.Unrestricted`, and the recipe
deliberately does not grant that — it would make the token strictly more
powerful than the ssh access it exists to complement.

What the token *can* do is bind an address to a VMID, via
...
qm terminal <vmid>   # the serial console the guest is configured for
```

That is a genuine limit of the split, not an oversight: the same property that
makes the token safe to keep on a workstation is the one that stops it reading
