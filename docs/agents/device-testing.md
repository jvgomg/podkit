...
Full design + error semantics + scope boundaries in [docs/architecture/testing/vm-build-orchestration.md](../architecture/testing/vm-build-orchestration.md).
...
See [`docs/persona-capture-playbook.md`](../persona-capture-playbook.md) for the full step-by-step (the playbook supersedes the auto-capture script originally planned in TASK-321.02). High-level:
...
See [docs/agents/testing.md](testing.md) §"Per-OS Test Tagging" for the exact `describe.skipIf` pattern and the `console.log` placement that makes skips visible in CI output.
...
- [docs/agents/testing.md](testing.md) — test stack overview, tagging convention, quick-reference commands
