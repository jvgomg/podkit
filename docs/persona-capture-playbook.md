...
| File | Why |
|------|-----|
| `docs/adr/adr-017-device-persona-fixtures.md` § "DevicePersona schema" + § "Schema v2 — May 2026" | Authoritative schema + capture rationale + v2 migration notes |
...
| `docs/test-devices.md` | Inventory of the user's hardware with model numbers, serial-suffix mappings, and notes per device |
| `docs/sysinfo-captures/` | Pre-existing SysInfoExtended XML payloads for 7 of the 9 iPods — reuse these; do NOT re-capture |
| `docs/device-identification.md` | USB-inquiry vs SCSI-fallback boundary + transport semantics |
...
|---|-----------|--------|--------------|-------------------|
| 1 | `ipod-video-5g-iflash-1tb` | iPod 5G Video (iFlash 1TB mod) | SCSI-fallback | `docs/sysinfo-captures/ipod-5g-video-iflash-1tb.xml` |
| 2 | `ipod-mini-2g-pink` | iPod mini 2G (4GB Pink) | SCSI-fallback | `docs/sysinfo-captures/mini-2g.xml` |
| 3 | `ipod-nano-2g-green` | iPod nano 2G (4GB Green) | SCSI-fallback | `docs/sysinfo-captures/nano-2g-4gb-green.xml` |
| 4 | `ipod-nano-3g-black` | iPod nano 3G (8GB Black) | USB-inquiry (boundary) | `docs/sysinfo-captures/nano-3g-8gb-black.xml` |
| 5 | `ipod-nano-4g-black` | iPod nano 4G (8GB Black) | USB-inquiry | `docs/sysinfo-captures/nano-4g-8gb-black.xml` |
| 6 | `ipod-nano-7g-space-gray` | iPod nano 7G (16GB Space Gray) | USB-inquiry | `docs/sysinfo-captures/nano-7g-16gb-usb.xml` (preferred — also has `-scsi.xml`) |
| 7 | `ipod-nano-7g-blue` | iPod nano 7G #2 (16GB Blue) | USB-inquiry | `docs/sysinfo-captures/nano-7g-16gb-blue-usb.xml` |
...
│   └── sysinfo-extended.xml      # symlink or copy from docs/sysinfo-captures/ (iPod only)
└── (for echo-mini only)
...
2. Have `docs/test-devices.md` open — the agent will cross-reference model number / serial suffix against it for each device.
...
For iPods 1–7 in the inventory, do **not** re-capture — the XML already exists in `docs/sysinfo-captures/`. Symlink or copy it into `raw/`:

```bash
cp docs/sysinfo-captures/<existing-file>.xml \
...
The exact `unsupportedReason` text comes from `packages/devices-ipod/src/unsupported.ts` for the iPod touch + shuffle cases; for `non-ipod-usb-disk`, doctor doesn't even reach the unsupported-PID gate (per the notes in `docs/test-devices.md`) — assert the rejection at the `device add` layer instead.
...
- Origin: `docs/sysinfo-captures/<filename>.xml`
- Captured (originally): <date from test-devices.md if available>
- Inquiry transport used: USB | SCSI | both

## Cross-references
- Inventory entry: `docs/test-devices.md` § "<device section heading>"
- ADR-017: `docs/adr/adr-017-device-persona-fixtures.md`
- Capture playbook: `docs/persona-capture-playbook.md` (this file)
```
...
Three shuffles (2G/3G/4G) have since been acquired — see `docs/test-devices.md` — so this persona can be re-captured from hardware when someone gets to it. Until then, synthesise:
...
- `sysInfoExtendedXml: null` (synthesis convenience only — a shuffle 2G does expose SIE; see `docs/sysinfo-captures/shuffle-2g-1gb-pink.xml`)
...
5. Update `docs/test-devices.md` — append a "Persona captured: `<persona-id>` (YYYY-MM-DD)" line under each device's section.
...
| SIE XML doesn't parse / doctor refuses the persona | The pre-existing XML in `docs/sysinfo-captures/` may have been captured against an older parser | Re-capture via `podkit device repair sysinfo-extended --device <name>` and update both locations |

## Done criteria

- 11 persona directories under `test-packages/device-testing/src/personas/`
- Registry index populated; package builds + tests pass
- `docs/test-devices.md` updated with capture timestamps
