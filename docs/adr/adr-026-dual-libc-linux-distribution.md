...
Refines the distribution mechanics of [ADR-021](./adr-021-cli-bun-binary-distribution.md) (CLI ships as a Bun `--compile` binary).
...
## Related decisions
- [ADR-021](./adr-021-cli-bun-binary-distribution.md) — CLI distributes as a Bun `--compile` binary.
- [ADR-002](./adr-002-libgpod-binding.md) — in-process N-API bindings (the reason a native blob is per-libc).
