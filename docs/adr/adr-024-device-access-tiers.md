...
Extends the m-18 device-capability architecture of [ADR-014](./adr-014-device-capability-architecture.md) and the structured-identity fields of [ADR-020](./adr-020-ipod-identity-structured-fields.md). Supersedes the binary `IpodGeneration.supported` framing. Specified by **doc-056 (PRD: Device Access Tiers)**.
...
- **A `bdhs` parser.** Out of scope: podkit reads `iTunesDB`, not `iTunesSD`; the `bdhs` format is *documented* (see `docs/formats/itunessd-bdhs.md`), not parsed.

## References

- doc-056 — PRD: Device Access Tiers (specification)
- [ADR-014](./adr-014-device-capability-architecture.md) — Device Capability Architecture (m-18)
- [ADR-020](./adr-020-ipod-identity-structured-fields.md) — iPod Identity Structured Fields
- `docs/formats/itunessd-bdhs.md` — the `bdhs` iTunesSD format reference seeded by this investigation
