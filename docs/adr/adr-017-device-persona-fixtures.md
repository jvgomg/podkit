...
├── package.json
├── tsconfig.json
├── README.md                       # Cross-references docs/agents/device-testing.md
├── src/
...
The starter set is sized to cover the three principal inquiry/discovery paths (SCSI fallback, USB inquiry, mass-storage) using devices from the user's hardware inventory (`docs/test-devices.md`). Original MC297 entries were swapped because that model is not in the inventory.
...
Real SCSI VPD payloads, `lsblk` output, `system_profiler` JSON, and `diskutil` plists are captured from physical hardware using the existing `docs/sysinfo-captures/` workflow for VPD, plus new capture scripts at `test-packages/device-testing/scripts/capture-persona.ts` for the host-OS probe layers.
...
- [ADR-005](./adr-005-test-ipod-environment.md) — iPod test environment; `gpod-testing` templates remain separate for database-layer tests; this package covers the USB/SCSI/probe/system-environment layers
- [ADR-014](./adr-014-device-capability-architecture.md) — Device capability architecture; `expectedCapabilities` in each persona is a `DeviceCapabilities` record produced by `resolveCapabilities`
- [ADR-016](./adr-016-linux-vm-test-harness.md) — Linux VM test harness; this ADR provides the fixture and harness package that ADR-016's device test stack consumes
...
- `docs/test-devices.md` — canonical hardware inventory; each persona cross-references the matching entry
- `docs/sysinfo-captures/` — existing SysInfoExtended XML captures reused as fixture source
- `docs/agents/device-testing.md` — agent guide for the device test stack, persona capture, and runner ops (created by TASK-321.08)
