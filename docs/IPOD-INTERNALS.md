...
This document covers the internal structure of iPod devices, focusing on the database format, file organization, and artwork handling.
...
libgpod parses model numbers with this logic (from `itdb_device.c`):
...
|--------|------|-------------|
| 0 | 4 | Magic: "mhbd" |
| 4 | 4 | Header length (typically 104) |
| 8 | 4 | Total database length |
| 12 | 4 | Unknown |
| 16 | 4 | Version number |
| 20 | 4 | Number of children |
| 24 | 8 | Database ID |
| ... | ... | More fields |
...
| 0 | 4 | Magic: "mhit" |
| 4 | 4 | Header length |
| 8 | 4 | Total length including children |
| 12 | 4 | Number of children (mhod strings) |
| 16 | 4 | Track ID |
| 20 | 4 | Visible (1) or hidden (0) |
| 24 | 4 | File type (see below) |
| ... | ... | Many more fields |
...
```c
// Set artwork from JPEG/PNG file (libgpod handles conversion)
...
### libgpod Device Detection

```c
// Check if path is an iPod
...
# Then re-add all tracks with libgpod/podkit
```
...
Files in `Music/Fxx/` not referenced by iTunesDB:

```bash
# Find orphaned files (compare to database)
# This is complex - libgpod handles this internally
```
...
- [iPod Linux Project (archived)](https://web.archive.org/web/20120301000000*/ipodlinux.org)
