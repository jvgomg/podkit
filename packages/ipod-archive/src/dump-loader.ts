 ...
 * (the named archive dir containing `raw/`, or a directory that itself
 * contains `iPod_Control`), opens the iTunesDB through libgpod-node's
 * `Database.open()` (which calls `itdb_parse()` with no device gate), and
 * resolves the device identity via {@link resolveDumpIdentity} — name from the
 * iTunesDB master playlist, model from a SysInfoExtended (on-disk in the dump,
 * else the captured sidecar) run through `resolveIpodModel`, else libgpod
 * capabilities. See `device-identity.ts` for the full precedence.
 *
 * @module
 */
...
import { RAW_DUMP_SUBDIR, LEGACY_RAW_DUMP_SUBDIR } from './run-dump.js';
 ...
 * 2. `dumpDir/raw/` (the stage-1 named-archive layout);
 * 3. `dumpDir/raw dump/` (the legacy layout, for dumps taken before the rename);
 * 4. otherwise throws `DUMP_NOT_READABLE`.
 */
async function resolveIpodRoot(dumpDir: string): Promise<string> {
  if (await hasIpodControl(dumpDir)) return dumpDir;

  for (const subdir of [RAW_DUMP_SUBDIR, LEGACY_RAW_DUMP_SUBDIR]) {
    const candidate = join(dumpDir, subdir);
    if (await hasIpodControl(candidate)) return candidate;
  }
 ...
 *
 * Identity is resolved by {@link resolveDumpIdentity}: name from the iTunesDB
 * master playlist, model from a SysInfoExtended (on-disk, else the captured
 * `podkit-sysinfo-extended.xml` sidecar) via `resolveIpodModel`, else libgpod.
 *
...
export async function loadDump(dumpDir: string): Promise<LoadedDump> {
  ...
  // artifact is read from `dumpDir` (the named dir), not `ipodRoot` (`raw/`).
  try {
