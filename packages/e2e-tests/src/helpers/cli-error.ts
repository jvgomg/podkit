 ...
 *
 * Subprocess flavor of the in-process helper at
 * `packages/podkit-cli/src/test-utils/cli-error.ts`.
 *
 * Spawns the CLI, captures stdout + stderr + exitCode, and asserts the
 * stdout contains a `CliErrorOutput` payload matching expectations.
 */
...
/**
 * Run the CLI and assert the failure shape. Returns the parsed payload
 * and raw `CliResult` so callers can do additional inspection.
 *
 * @example
 * ```ts
