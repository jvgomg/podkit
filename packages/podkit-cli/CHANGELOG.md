...
- [`2a566a6`](https://github.com/jvgomg/podkit/commit/2a566a679f7c28f0a552e9466248b101fffa34b9) Thanks [@jvgomg](https://github.com/jvgomg)! - Make the quality preset a real ceiling on macOS by driving `aac_at` in ABR mode
...
- [`c5cba69`](https://github.com/jvgomg/podkit/commit/c5cba6998283663b42659f02b17b194ab256c137) Thanks [@jvgomg](https://github.com/jvgomg)! - Polish on the convergent-metadata work (TASK-327 follow-up):
  ...
  - **Docs.** `transferMode` now has a dedicated section in `packages/docs-site/src/content/docs/reference/config-file.md` explaining the iPod vs mass-storage contract and migration churn. `pathTemplate` (from the prior release) and `PODKIT_PATH_TEMPLATE` are now documented in the config reference and environment-variables reference.
...
- [`44c5ac1`](https://github.com/jvgomg/podkit/commit/44c5ac13800aac55db341bf196e39a0fedf157f7) Thanks [@jvgomg](https://github.com/jvgomg)! - Stop a concurrent sync's debris sweep from deleting another sync's in-flight transcodes.
  ...
  An unmarked scratch directory is now left alone until it has gone a minute untouched, which no live session ever does. A directory whose owner is a _dead_ process is still reclaimed on sight, so a killed sync's leftovers are cleared by the next run exactly as before.
