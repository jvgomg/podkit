/**
 * Integration tests for IpodDeviceAdapter sync tag round-trip
 *
 * These tests verify that sync tags survive a full write → save → reopen → read
 * cycle through the iPod database. They complement the unit tests in
 * sync-tags.test.ts and the mass-storage integration tests.
 *
 * ## Requirements
