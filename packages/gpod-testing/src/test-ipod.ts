...
import { templatePath, TEMPLATE_MODELS } from './templates';

const DEFAULT_NAME = 'Test iPod';
const TEMPLATE_MODEL_SET = new Set<string>(TEMPLATE_MODELS);

const templateAvailability = new Map<string, boolean>();
async function hasTemplate(model: IpodModelNumber): Promise<boolean> {
  if (!TEMPLATE_MODEL_SET.has(model)) return false;
  const cached = templateAvailability.get(model);
  if (cached !== undefined) return cached;
  try {
    const s = await stat(templatePath(model));
    const ok = s.isDirectory();
    templateAvailability.set(model, ok);
    return ok;
  } catch {
    templateAvailability.set(model, false);
    return false;
  }
}
...
export async function createTestIpod(
  ...
  const firewireId = options.firewireId ?? TEST_FIREWIRE_GUID;
  ...
  // Fast path: copy a pre-built template when the caller hasn't customised
  // anything that would diverge from it. ~5ms vs ~300ms subprocess spawn.
  // Templates are generated by the gpod-testing#generate-templates turbo task.
  // Set PODKIT_DISABLE_TEMPLATE_CACHE=1 to force the slow subprocess path
  // (used to benchmark the speedup).
  const useTemplate =
    process.env.PODKIT_DISABLE_TEMPLATE_CACHE !== '1' &&
    name === DEFAULT_NAME &&
    firewireId === TEST_FIREWIRE_GUID &&
    (await hasTemplate(model));

  if (useTemplate) {
    await cp(templatePath(model), ipodPath, { recursive: true });
  } else {
    await gpodTool.init(ipodPath, { model, name, firewireId });
  }
