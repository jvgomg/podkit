 ...
 * `packages/device-testing/{src,dist}/runners/lima-test-vm.ts` so the repo
...
export interface CreateLimaTestVmRuntimeOpts {
   ...
   * `tools/device-testing/dummy-hcd/dist/dummy-hcd-daemon-linux-<arch>`.
   */
      ...
      if (fs.existsSync(daemonPath)) {
        ...
        console.warn(
          `[lima-test-vm] dummy-hcd-daemon binary not found at ${daemonPath} ` +
            `— run \`bash tools/device-testing/dummy-hcd/scripts/build.sh\` to produce one.`
        );
      }
