...
status: Done
assignee: []
created_date: '2026-03-22 15:09'
updated_date: '2026-03-22 19:41'
labels:
  - bug
  - ux
dependencies: []
...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 Video sync progress display shows correct completed count (increments once per finished operation, not per progress event)
- [x] #2 Music sync progress display also counts correctly through the unified executor path
<!-- AC:END -->
...
Add `completedCount` field to `ExecutorProgress` type, maintained by UnifiedExecutor. CLI uses this instead of inferring completion from phase strings. Fixes overcounting for both video and music paths.

## Completed (2026-03-22)

Added `completedCount` field to `ExecutorProgress` type, maintained by UnifiedExecutor and DefaultSyncExecutor. CLI now uses `progress.completedCount` instead of manual phase-based counting. Fixed overcounting in both video sync functions and the music sync function.
<!-- SECTION:NOTES:END -->
