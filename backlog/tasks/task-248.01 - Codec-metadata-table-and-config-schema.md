...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Codec metadata table exists as a shared constant with entries for AAC, ALAC, Opus, MP3, FLAC — each with extension, FFmpeg format flag, filetype label, sample rate, and type
- [ ] #2 Config types accept `[codec]` section with `lossy` and `lossless` fields at global and per-device levels
- [ ] #3 Single string values are normalized to arrays during config loading
- [ ] #4 Config validation rejects unknown codec identifiers
- [ ] #5 Existing configs without `[codec]` section load successfully using default stacks
- [ ] #6 Example config documents the default codec stacks with explanations of each codec
- [ ] #7 `encoding` field docstrings and CLI help text updated to say codec-generic (not AAC-specific)
- [ ] #8 Config loader and writer tests pass for new schema
<!-- AC:END -->
