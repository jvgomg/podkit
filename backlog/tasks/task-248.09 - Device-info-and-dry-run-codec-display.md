...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 `podkit device info` shows codec preference list with ✓/✗ per codec based on device support
- [ ] #2 `podkit device info` shows incompatibility message when no codec in preference list is supported
- [ ] #3 `podkit sync --dry-run` shows resolved codec with preference chain in summary
- [ ] #4 `podkit sync --dry-run` shows codec change count and direction when codec changes detected
- [ ] #5 `podkit sync` and `podkit sync --dry-run` error and exit with helpful message when no codec matches, listing device-supported codecs
<!-- AC:END -->
