...
**Stage 1 (local build) vs Stage 2 (pull the real GHA image).** By default the harness builds the image locally from the current musl binaries — the fast dev loop. To gate against the **actual GHA-built artifact** instead, set `PODKIT_DOCKER_DIST_IMAGE=ghcr.io/jvgomg/podkit:edge`; both this stage and the tier-4 loopback stage then `pull` that tag rather than building (one env var, both surfaces, via `ensurePodkitImageInVm` / `ensurePodkitImageOnHost`). The `ghcr.io/jvgomg/podkit` package is **public**, so the pull is anonymous — no login/token. The moving `:edge` tag is built on every push to `main` by `.github/workflows/docker-edge.yml` (arm64-only, reusing `docker.yml`; never the release tags), and untagged overwrites are pruned weekly by `.github/workflows/docker-prune.yml`. Full loop in `agents/docker.md` → "Gating against the real GHA-built image (`:edge`)".
