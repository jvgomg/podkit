...
**Stage 1 (local build) vs Stage 2 (pull the real GHA image).** By default the harness builds the image locally from the current musl binaries — the fast dev loop. To gate against the **actual GHA-built artifact** instead, set `PODKIT_DOCKER_DIST_IMAGE=ghcr.io/jvgomg/podkit:rc`; both this stage and the tier-4 loopback stage then `pull` that tag rather than building (one env var, both surfaces, via `ensurePodkitImageInVm` / `ensurePodkitImageOnHost`). The `ghcr.io/jvgomg/podkit` package is **public**, so the pull is anonymous — no login/token. The moving `:rc` tag is the **release candidate**: it is built by `.github/workflows/verify-release.yml` when the open "Version Packages" PR runs its verification build (multi-arch, reusing `docker.yml`; never the release tags; the push gated to non-fork PRs), and untagged overwrites are pruned weekly by `.github/workflows/docker-prune.yml`. The former per-push `:edge` image and its `docker-edge` workflow are **retired** — `bun run quality` builds the image locally and `bun run quality:rc` pulls `:rc`, so `:edge` no longer had a consumer. `bun run quality:rc` is the one command that discovers the release-candidate build, fetches its binaries, and pulls `:rc` across every surface (agents/testing.md → "The two quality mirrors"); full manual loop in `agents/docker.md` → "Gating against the real GHA-built image (`:rc`)". Fidelity caveat: `:rc` is the same recipe + shared cache as release — functionally the release bytes, but not bit-identical (e.g. the image build-date label differs); CI-fidelity gating exists only during the release-candidate window.
